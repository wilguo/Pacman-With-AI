...
class ExactInference(InferenceModule):
        ...
        * When a ghost is captured by Pacman, all beliefs should be
        ...
        if noisyDistance != None :
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    positionBelief = self.beliefs[p]
                    positionEmissionModel = emissionModel[trueDistance]
                    allPossible[p] = positionBelief * positionEmissionModel
        else:
            jailPosition = self.getJailPosition()
            allPossible[jailPosition] = 1
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # for old in self.legalPositions:
        #     newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, old))
        #     for newPos, prob in newPosDist.items():
        #         allPossible[newPos] += prob*self.beliefs[old]


        for i in range(len(self.legalPositions)):
            ghostPosition = self.setGhostPosition(gameState, self.legalPositions[i])
            newPositionD = self.getPositionDistribution(ghostPosition)
            for item in newPositionD.items():
                allPossible[item[0]] += item[1] * self.beliefs[self.legalPositions[i]]
        self.beliefs = allPossible
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        size = len(legalPositions)

        for i in range(len(legalPositions)):
            for j in range(numParticles / size):
                particles.append(legalPositions[i])
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance != None:
            ...
            for i in range(len(legalPositions)):
                index = legalPositions[i]
                eModel = emissionModel[util.manhattanDistance(legalPositions[i], pacmanPosition)]
                possible[index] += eModel * oldB[index]

            valueList = possible.values()

            if any(valueList):
                sampleList = []
                for i in range(0, numParticles):
                    sample = util.sample(possible)
                    sampleList.append(sample)

                self.particles = sampleList
            else:
                self.initializeUniformly(gameState)
        else:
            particles = [jailPosition] * self.numParticles
            self.particles = particles
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(len(particles)):
            position = self.setGhostPosition(gameState, particles[i])
            particleList.append(util.sample(self.getPositionDistribution(position)))
    ...
    def getBeliefDistribution(self):
        ...
        for i in range(len(self.particles)):
            beliefDistribution[self.particles[i]] += 1
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = []
        particles = self.particles
        itr = itertools.product(legalPositions, repeat=self.numGhosts)
        possiblePositions = list(itr)
        c = 0

        while c < numParticles:
            for i in range(len(possiblePositions)):
                if c >= self.numParticles:
                    break
                else:
                    particles.append(possiblePositions[i])
                    c += 1
        ...
        for i in range(len(self.particles)):
            ...
            while j < self.numGhosts:
                if noisyDistances[j] != None:
                    p = p * emissionModels[j][util.manhattanDistance(self.particles[i][j], pacmanPosition)]
                else:
                    partical = self.particles[i]
                    self.particles[i] = self.getParticleWithGhostInJail(partical, j)

                j += 1

            possibleList[self.particles[i]] += p

        if any(possibleList.values()):
            possibleList.normalize()
            tempList = [util.sample(possibleList) for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            while i < self.numGhosts:
                g = self.ghostAgents[i]
                p = setGhostPositions(gameState, newParticle)
                newParticle[i] = util.sample(getPositionDistributionForGhost(p, i, g))
                i += 1
            "*** END YOUR CODE HERE ***"
