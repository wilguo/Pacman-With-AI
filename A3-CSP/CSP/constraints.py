...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        scope = [qi, qj]
        validAssignments = []
        for iqueen in qi.domain():
            for jqueen in qj.domain():
                if iqueen != jqueen and abs(iqueen - jqueen) != abs(i - j):
                    validAssignments.append([iqueen, jqueen])
        TableConstraint.__init__(self, name, scope, validAssignments)
...
class NValuesConstraint(Constraint):
    ...
    def check(self):
        ...
        for var in self.scope():
            if self.includes(self._required, var.getValue()):
                counter += 1
        return self._lb <= counter and counter <= self._ub
    ...
    def hasSupport(self, var, val):
        ...
        def checkBounds(l):
            ...
            for value in value_list:
                if self.includes(self._required, value):
                    counter += 1
            return self._lb <= counter and counter <= self._ub

        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], checkBounds)
        return x
