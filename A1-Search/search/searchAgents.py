...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            # If next move pacman doesn't hit a wall
            if not hits_wall:
                # If next move is not one of the corners
...
def cornersHeuristic(state, problem):
    ...
    while not len(corners) == 0:
        curr_position_x, curr_position_y = reference_point

        distances = []

        for goal_position_x, goal_position_y in corners:
            dx = abs(curr_position_x - goal_position_x)
            dy = abs(curr_position_y - goal_position_y)
            manhattan = dx + dy
            distances.append(manhattan)

        heuristic += min(distances)
        closest = distances.index(min(distances))
        reference_point = corners[closest]
        corners.remove(reference_point)
...
def foodHeuristic(state, problem):
    ...
    distance_list = []
    curr_position_x, curr_position_y = position

    for goal_position_x, goal_position_y in food_list:
        dx = abs(curr_position_x - goal_position_x)
        dy = abs(curr_position_y - goal_position_y)
        manhattan = dx + dy

        if goal_position_x == curr_position_x or goal_position_y == curr_position_y:
            manhattan = 1

        distance_list.append(manhattan)
